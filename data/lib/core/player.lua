...
function Player.transferMoneyTo(self, target, amount)
	...
	if targetPlayer then
		...
		local query_town =
			db.storeQuery("SELECT `town_id` FROM `players` WHERE `name` = " .. db.escapeString(target) .. " LIMIT 1;")
...
function Player.hasRookgaardShield(self)
	...
	return self:getItemCount(3412) > 0 or self:getItemCount(3426) > 0 or self:getItemCount(3411) > 0 or
		self:getItemCount(3410) > 0 or
		self:getItemCount(3430) > 0
end
...
function Player.isMage(self)
	return table.contains(
		{VOCATION.ID.SORCERER, VOCATION.ID.MASTER_SORCERER, VOCATION.ID.DRUID, VOCATION.ID.ELDER_DRUID},
...
function Player.getAccountStorage(self, accountId, key, forceUpdate)
	...
	local query =
		db.storeQuery(
		"SELECT `key`, MAX(`value`) as value FROM `player_storage` WHERE `player_id` IN (SELECT `id` FROM `players` WHERE `account_id` = " ..
			accountId .. ") AND `key` = " .. key .. " GROUP BY `key` LIMIT 1;"
	)
	if query ~= false then
