...
npcConfig.shop = {
	...
	{itemName = "great mana potion", clientId = 238, clientname = "great mana potion", buy = 144},
...
local function handleBankActions(npc, creature, message)
			...
			if player:depositMoney(count[playerId]) then
				npcHandler:say(
					"Alright, we have added the amount of " ..
						count[playerId] .. " gold to your {balance}. You can {withdraw} your money anytime you want to.",
					npc,
					creature
				)
			else
		...
		if MsgContains(message, "yes") then
			if player:getFreeCapacity() >= getMoneyWeight(count[playerId]) then
				if not player:withdrawMoney(count[playerId]) then
					...
					npcHandler:say(
						"Here you are, " .. count[playerId] .. " gold. Please let me know if there is something else I can do for you.",
						npc,
						creature
					)
				end
		...
		if MsgContains(message, "yes") then
			npcHandler:say(
				"We have placed an order to transfer " ..
					count[playerId] ..
						" gold from your guild account to guild " .. transfer[playerId] .. ". Please check your inbox for confirmation.",
				npc,
				creature
			)
			local guild = player:getGuild()
	...
	elseif npcHandler:getTopic(playerId) == 1632 then
		...
		if MsgContains(message, "yes") then
			if not player:transferMoneyTo(transfer[playerId], count[playerId]) then
				...
				npcHandler:say(
					"Very well. You have transferred " .. count[playerId] .. " gold to " .. transfer[playerId] .. ".",
					npc,
					creature
				)
	...
	elseif npcHandler:getTopic(playerId) == 1636 then
		if getMoneyCount(message) < 1 then
			...
			npcHandler:say(
				"So you would like me to change " ..
					count[playerId] .. " of your platinum coins into " .. count[playerId] * 100 .. " gold coins for you?",
				npc,
				creature
			)
	...
	elseif npcHandler:getTopic(playerId) == 1638 then
		if getMoneyCount(message) < 1 then
			...
			npcHandler:say(
				"So you would like me to change " ..
					count[playerId] * 100 .. " of your platinum coins into " .. count[playerId] .. " crystal coins for you?",
				npc,
				creature
			)
			npcHandler:setTopic(playerId, 1641)
	...
	elseif npcHandler:getTopic(playerId) == 1640 then
		if getMoneyCount(message) < 1 then
			...
			npcHandler:say(
				"So you would like me to change " ..
					count[playerId] .. " of your crystal coins into " .. count[playerId] * 100 .. " platinum coins for you?",
				npc,
				creature
			)
			npcHandler:setTopic(playerId, 1641)
