...
-- Move file to this location: data/scripts/login.lua
...
local loginWebService = GlobalEvent("loginWebService")
function loginWebService.onStartup()
    print(" ")
	print("=============================")
    ...
    elseif configSQL.SERVER_NAME ~= configLua.SERVER_NAME then
        db.query("UPDATE `global_storage` SET `value`=".. db.escapeString(configLua.SERVER_NAME) .." WHERE `key`='SERVER_NAME';")
    ...
    elseif configSQL.IP ~= configLua.IP then
        db.query("UPDATE `global_storage` SET `value`=".. db.escapeString(configLua.IP) .." WHERE `key`='IP';")
    ...
    elseif configSQL.GAME_PORT ~= tostring(configLua.GAME_PORT) then
        db.query("UPDATE `global_storage` SET `value`=".. db.escapeString(configLua.GAME_PORT) .." WHERE `key`='GAME_PORT';")
        print("= Updated [GAME_PORT] FROM [" .. configSQL.GAME_PORT .. "] to [" .. configLua.GAME_PORT .. "]")
    end
    if #inserts > 0 then
        db.query("INSERT INTO `global_storage` (`key`,`value`) VALUES "..table.concat(inserts,',')..";")
